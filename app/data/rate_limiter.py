...
# ============ 频率配置（保守，针对共享 token）============
# tushare 5000积分纯个人使用是 500次/分钟，但多人共享时每人能用的是这个数除以人数
# 我们假设最多5个人共享，单人按 100次/分钟保守，再除以接口数取均值

INTERFACE_LIMITS: dict[str, dict] = {
    # 默认（用于未明确配置的接口）
    "_default":     {"per_min": 30,  "min_interval": 2.0},

    # 行情类（最常用）
    "daily":        {"per_min": 50,  "min_interval": 1.2},
    "daily_basic":  {"per_min": 50,  "min_interval": 1.2},
    "moneyflow":    {"per_min": 40,  "min_interval": 1.5},
    "adj_factor":   {"per_min": 40,  "min_interval": 1.5},
    "index_daily":  {"per_min": 30,  "min_interval": 2.0},

    # 元数据（很少调用）
    "stock_basic":  {"per_min": 10,  "min_interval": 6.0},
    "trade_cal":    {"per_min": 10,  "min_interval": 6.0},

    # 概念板块
    "concept":          {"per_min": 10, "min_interval": 6.0},
    "concept_detail":   {"per_min": 30, "min_interval": 2.0},

    # 其他
    "top_list":     {"per_min": 20,  "min_interval": 3.0},
    "top_inst":     {"per_min": 20,  "min_interval": 3.0},
}


# 全局所有接口共用的间隔（防止突发，最低保护）
GLOBAL_MIN_INTERVAL = 0.8
